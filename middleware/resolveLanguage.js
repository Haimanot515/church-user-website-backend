...
const resolveLanguage = async (req, res, next) => {
  try {
    ...
    let languageDoc = await languageService.findByCode(code);
    if (!languageDoc) {
      // fallback to default language if the requested one doesn't exist
      languageDoc = await languageService.findByCode(DEFAULT_LANGUAGE_CODE);
    }
    req.language = languageDoc ? languageDoc.id : null;
    next();
  } catch (err) {
    res.status(500).json({
      message: err.message
