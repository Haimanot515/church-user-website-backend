...
exports.createThread = async (req, res) => {
  try {
    ...
    let thread = await contactService.findThreadByEmail(email);

    if (!thread) {
      thread = await contactService.createThread(name, email);
    }

    const newMessage = await contactService.createMessage(thread.id, message);

    thread = await contactService.updateThreadAfterMessage(thread.id, message);
